...
One Telegram Bot identity serves two isolated contexts behind the authenticated primary webhook: authorized private chats enter Admin, and only the configured forum supergroup enters Support Topics. Unified Admin supports guarded one-click online Bot rotation. A genuinely new candidate Bot is validated against the forum and topic-management permission, confirmation creates a fresh webhook identity with a clean pending-update boundary, activates the next `TELEGRAM_SUPPORT_PROFILE` generation, then disables both the previous Support webhook and the legacy Admin webhook. Full success means only the new Bot remains active and no manual webhook refresh or separate retirement action is required. If retirement is incomplete after activation, the new generation remains authoritative and authenticated Admin state retries only the remaining old-webhook retirements. Legacy route/code and bootstrap credentials may remain offline for break-glass recovery, while `TELEGRAM_SUPPORT_PROFILE` generation/version semantics continue to preserve Queue identity and stale-generation fencing. No D1 migration is added. Staging and Production switching remain separate Owner-authorized work.

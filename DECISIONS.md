...
**Decision:** A `conversation_status_changed` webhook triggers reconciliation but its delivery identity and arrival order do not determine whether `open` or `resolved` is newer. The Queue consumer reads the current Chatwoot conversation status, compares it with canonical D1 topic state, and only then prepares a Telegram close/reopen operation. New lifecycle operations use one ordered per-conversation namespace across both directions; same-state events reuse the current operation, while the next opposite transition advances the sequence. Effective delivery is repaired through the existing outbound domain resolver before another transition is considered.

**Concurrency and failure rule:** After a successful Telegram operation and D1 repair, the consumer reads Chatwoot again for a bounded convergence pass. This covers provider state changes during the Telegram request without treating local timestamps or D1 version increments as provider chronology. Active `SENDING` leases contend normally. `SENT` is repaired provider-free. `AMBIGUOUS` and `FAILED_FINAL` are not automatically replayed. Managed lifecycle operations are not eligible for generic manual retry because that path cannot prove current Chatwoot state around the Telegram boundary. Query failure, malformed or unsupported Chatwoot status and exhausted convergence fail closed before another visible action. An older managed lifecycle operation, including any existing manual-retry descendant, cannot mutate D1 after a newer managed lifecycle operation exists.

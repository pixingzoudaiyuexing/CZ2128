...
**Stale convergence rule:** When freshness fencing permanently abandons a historical AI trigger, its deterministic `ai_reply:<eventId>` and `ai_tg_mirror:<eventId>` operations must also converge. Identity/evidence-valid `PENDING`, and `FAILED_RETRYABLE` rows whose persisted request/response evidence proves an observed 429, transition through state-fenced D1 updates to `FAILED_FINAL / DISCARDED_STALE`. Attempts and request/response evidence are preserved; retry and lease fields are cleared. Each actual change has one deterministic adjacent reliability audit. `SENT` and existing `FAILED_FINAL` remain unchanged. `SENDING`, `AMBIGUOUS`, malformed evidence or a concurrent claim fail closed so canonical event completion cannot resolve the DLQ while unsafe work remains.

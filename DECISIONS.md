...
**Status:** COMPLETE / FROZEN / MERGED. Phase 4B-2C overall is COMPLETE / FROZEN. Phase 4B-3 is COMPLETE / FROZEN / MERGED. Phase 4B-4A is COMPLETE / FROZEN / MERGED. Phase 4B-4B is ACCEPTED / COMPLETE / FROZEN / MERGED. Phase 4B-5 is ACCEPTED / COMPLETE / FROZEN / MERGED; 4C remains NOT STARTED / PREPARATION ONLY.
...
**Status:** APPROVED / COMPLETE / FROZEN / MERGED. Phase 4B-4 overall is COMPLETE / FROZEN / MERGED. Phase 4B-4B is ACCEPTED / COMPLETE / FROZEN / MERGED. Phase 4B-5 is ACCEPTED / COMPLETE / FROZEN / MERGED; Phase 4C remains NOT STARTED / PREPARATION ONLY.

## D-029 — Recover only reconstructable AI triggers from durable state

**Decision:** Phase 4B-4B supports explicit recovery only for `internal / ai_trigger`. It is not generic replay. The original `source_event_ref` is reused exactly, and the event payload contains only the validated canonical conversation ID and durable Chatwoot message reference. Eligibility requires an OPEN `cz2128-dlq` receipt, matching reclaimable `event_receipts`, the newest durable customer text, an existing matching `ai_runs` row, enabled AI, equal handoff epochs, no active generation owner and an existing deterministic Chatwoot AI operation with durable target evidence. Current configuration alone never proves a historical target. External messages, lifecycle events, `attachment_transfer` and quarantine evidence remain non-redrivable.

**AI, freshness and handoff rule:** Only due `FAILED_RETRYABLE` runs below the existing three-attempt limit and `SUCCESS` runs with durable response text are eligible. Attempts and retry history are never reset. Retry reclaim requires the durable run epoch to equal the current conversation epoch inside the claim CAS. Recovery rechecks newest durable customer text at consumer entry, atomically when saving a newly generated result, and immediately before each historical provider action. A newer handoff epoch permanently fences old work even after AI is re-enabled; a newer customer message discards retryable old work and prevents historical delivery. Historical SUCCESS remains truthful and is not regenerated.

**Outbound rule:** Normal fresh AI processing prepares the deterministic Chatwoot operation and immutable target evidence before crossing the AI provider boundary. Historical redrive may never create a missing Chatwoot operation; legacy runs without that evidence fail closed even when the current endpoint happens to match. Provider-visible continuation permits only `PENDING` or due in-budget `FAILED_RETRYABLE` with stored evidence matching the current target. `SENT` is provider-free and may converge without requiring the old endpoint to remain current. A missing Telegram mirror is never created during historical recovery; after confirmed Chatwoot delivery it is safely skipped. An existing matching mirror may continue, while target drift is terminated without a mirror send. `SENDING`, `AMBIGUOUS`, `FAILED_FINAL`, malformed evidence, identity conflicts, exhausted attempts and future retry deadlines retain their existing fail-closed semantics. AMBIGUOUS remains exclusively in reconciliation/Mark Delivered/Cancel/Manual Retry workflows.

**Abandoned-work convergence rule:** When freshness or human handoff permanently abandons AI work, deterministic `ai_reply:<eventId>` and `ai_tg_mirror:<eventId>` operations must converge with the same reason. Identity/evidence-valid `PENDING`, and `FAILED_RETRYABLE` rows whose persisted request/response evidence proves an observed 429, transition through state-fenced D1 updates to `FAILED_FINAL / DISCARDED_STALE` or `FAILED_FINAL / CANCELLED_BY_HANDOFF`. Attempts and request/response evidence are preserved; retry and lease fields are cleared. Each actual change has one deterministic adjacent reliability audit with reason-specific action. No-send cleanup validates historical evidence structurally but does not require it to match a later current conversation mapping. `SENT` and existing `FAILED_FINAL` remain unchanged. `SENDING`, `AMBIGUOUS`, malformed evidence or a concurrent claim fail closed so canonical event completion cannot resolve the DLQ while unsafe work remains. A late generation owner never cleans the shared deterministic operation after a newer valid generation has replaced it.

**SENT integrity rule:** Abandoned-work convergence accepts `SENT` as terminal delivery truth only when `provider_message_ref` is present and passes the same bounded identity rule used by redrive eligibility. This check applies both during the initial operation scan and after a cleanup CAS loses to a concurrent `SENT` transition. Malformed SENT is preserved but fails closed; it cannot trigger provider-free domain repair, event completion or DLQ resolution. Existing `FAILED_FINAL` remains a non-delivery terminal state rather than delivery proof and is not given speculative provider-reference constraints.

**Admin, audit and Queue rule:** The existing private Admin Bot exposes the action only from eligible D1 receipt inspection. Confirmation revalidates through the core service. A deterministic `DLQ_RECEIPT / DLQ_REDRIVE_REQUESTED` audit records one operator intent per receipt and Admin update before the exact event is sent to the existing main Queue. Same-command retries do not send twice; distinct commands may enqueue identical physical copies because `event_receipts`, `ai_runs`, generation fencing and deterministic outbound operation IDs own idempotency. The DLQ receipt remains OPEN until canonical processing resolves it.
...
## D-030 — Reconcile Chatwoot topic lifecycle from authoritative current state

**Decision:** A `conversation_status_changed` webhook triggers reconciliation but its delivery identity and arrival order do not determine whether `open` or `resolved` is newer. The Queue consumer reads the current Chatwoot conversation status, compares it with canonical D1 topic state, and only then prepares a Telegram close/reopen operation. New lifecycle operations use one ordered per-conversation namespace across both directions. The sequence ID deliberately excludes the target state, so concurrent opposite intents contend on one D1 primary key and one outbound lease. A typed identity collision is retryable only for that exact lifecycle slot; unrelated identity collisions remain hard failures. Effective delivery is repaired through the existing outbound domain resolver before another transition is considered.

**Concurrency and failure rule:** Before accepting a no-op from canonical D1 state, the consumer resolves or fences the current operation. `PENDING`, expired pre-request `SENDING` and due bounded `FAILED_RETRYABLE` continue only through the same ID. An opposite current intent may terminally supersede only a CAS-proven request that has not crossed the Provider boundary. Active started requests retain the lease; expired started requests become `AMBIGUOUS`. `SENT` is repaired provider-free. `AMBIGUOUS` and same-target `FAILED_FINAL` raise distinct explicit manual boundaries and never allocate a replacement ID. A definitively unsent/final operation permits only an opposite current state to proceed; it does not authorize retry of the failed target. `TOPIC_LIFECYCLE_SUPERSEDED_BEFORE_SEND` is the sole final-state exception: its zero-attempt CAS and adjacent audit prove that no Provider request occurred, so a later authoritative reversal may allocate the next sequence without rewriting history. Managed lifecycle operations are not eligible for generic manual retry.

**Cross-generation rule:** Before the first managed operation, a single pre-managed `SENT` lifecycle result may be repaired provider-free. Pre-managed `PENDING`, pre-request `SENDING` and bounded no-effect retry state continue through the original ID when the authoritative target is unchanged; they are CAS-terminated with audit only when the current target is opposite or managed mode already exists, so an old owner cannot later cross the Provider boundary. Started `SENDING` and `AMBIGUOUS` block managed effects. Once a managed root exists, pre-managed roots and children are fenced from D1 domain repair. If an already-started legacy request later becomes `SENT`, the next managed compensation records that legacy leaf as `parent_operation_id`, executes after the legacy completion and becomes the only newer state eligible for domain repair. Terminal or uncertain legacy histories are preserved; none are rewritten or retried.

**Compatibility and scope:** Existing pre-D-030 operation records remain readable and keep their historical status. The new sequence starts independently and uses the existing `outbound_operations`, `parent_operation_id`, target evidence, lease and domain-repair contracts. No migration `0006`, Durable Object, webhook authentication change, production action or staging deployment is introduced. Real out-of-order Provider validation remains a separate staging acceptance step after review and deployment authorization.

## D-031 — Gate staging AI tests by exact internal conversation identity

**Decision:** A deployment-only test switch may restrict all AI generation and delivery work to a strict JSON allowlist of canonical internal conversation UUIDs. The switch and allowlist are deliberately excluded from the Admin runtime control plane. The switch is opt-in; absent or exact `false` preserves existing production semantics. Any other switch value, or an enabled scope with a missing, empty, malformed, duplicate or invalid allowlist, denies all AI work.

**Execution rule:** Exact membership is rechecked at trigger enqueue, AI handler entry, immediately before the Provider attempt, on historical retry and durable-success recovery, during DLQ eligibility, and before each visible delivery. `PAUSED_OPERATOR` auto-resume cannot run outside scope. A denied unstarted run becomes `FAILED_FINAL / AI_SCOPE_DENIED` without changing `ai_mode`; deterministic unstarted outbound work receives the same final reason and audit. No new migration or AI status is required.

**Visible-side-effect rule:** Scope removal before Chatwoot delivery blocks that send. A durable AI `SUCCESS` remains truthful evidence that a Provider call completed and is not rewritten as if generation never happened. After Chatwoot is durably `SENT`, the same stable event may complete only provider-free domain repair and its deterministic Telegram mirror, even if scope then tightens. This preserves already-visible delivery evidence without opening a new generation or Chatwoot-send path.

**Reason:** Staging Provider acceptance must not expose every historical `ENABLED` conversation to AI calls, while production behavior must not silently change when the test-only deployment switch is absent.

## D-032 — Extend the private temporary attachment pipeline to Crisp

**Decision:** Crisp customer raster images, Telegram operator raster images, and Telegram operator ordinary files use the existing private R2 attachment subsystem, D1 attachment rows, Queue reference jobs and durable outbound-operation ledger. Migration `0006` extends only the attachment provider domain to include `crisp`; it does not add a second store or rewrite historical Chatwoot/Telegram rows. Crisp visitor file webhooks are accepted as image sources only for safe raster MIME types and an exact verified `https://storage.crisp.chat` locator. Direct Crisp ordinary-file upload is not treated as the canonical customer file path.

**Access rule:** Customer-visible image/download URLs are opaque 32-byte capability URLs whose plaintext token is never persisted; D1 stores only the token hash. The Worker serves raster images inline and ordinary files as downloads only while the attachment TTL is valid. The private R2 bucket remains non-public. Inline rendering is limited to JPEG, PNG, GIF, WebP and AVIF; active formats such as SVG are not served inline. Telegram Bot API token-bearing file URLs are source-only Worker internals and are never used as Crisp fallback links.

**Reliability rule:** Crisp attachment delivery remains one deterministic `SEND_ATTACHMENT` operation with Crisp target evidence and the existing numeric fingerprint echo suppression. Retryable responses reuse the same stored object and capability token. Response-loss ambiguity remains historical `AMBIGUOUS` and is never blindly resent. Because the plaintext capability is intentionally not stored, ambiguous Crisp attachment operations are not eligible for duplicate-risk manual retry. A possibly delivered ambiguous capability remains readable until its normal TTL so provider uncertainty does not invalidate a link that may already be visible.

**Scope:** This decision covers Crisp-05 Stage A only. Customer ordinary-file upload invitations remain a separate Stage B schema/security gate and must not be implemented as unrestricted public upload or by trusting client-supplied conversation/topic identity.

## D-033 — Bind Crisp ordinary-file upload to an operator-created temporary capability

**Decision:** Crisp ordinary customer files use a temporary upload invitation created from the already mapped Telegram conversation Topic, not a generic public upload API and not the direct Crisp ordinary-file webhook URL. `/upload` and `/upload_revoke` are accepted only from Telegram users already present in the bootstrap-only `ADMIN_TELEGRAM_USER_IDS` allowlist. The command is bound to the current Support Bot generation and Telegram update ordering before any capability is created or revoked. Migration `0007` adds `upload_invites` and `upload_invite_items` and extends `attachments.source_provider` with `upload`; the destination-provider domain remains unchanged.

**Capability rule:** `UPLOAD_CAPABILITY_SECRET` is bootstrap-only and must be stored as a Cloudflare secret. Invite and per-file download capabilities are domain-separated HMAC outputs; D1 stores only hashes. Every invite binds the canonical Conversation, Crisp Website/Session and Telegram group/topic, expires after 15 minutes, and has finite file/byte limits. Browser requests cannot choose or override routing identity. Upload POST revalidates the persisted binding, current Support Bot generation, OPEN Topic and authoritative Crisp state before accepting bytes. Images and active-content ordinary-file types are rejected from this path; accepted files remain in the existing private R2 attachment store and are exposed to operators only through forced-download capability URLs.

**Concurrency and reliability rule:** One `(invite, upload_id)` item is protected by a finite lease. SQLite atomically increments invite file/byte counters when an item becomes accepted and rejects limit-crossing races. A newer ordered upload command fences and revokes older active invitations; delayed older commands cannot supersede newer state. A visible Crisp invite uses one deterministic `UPLOAD_INVITE` outbound operation. Provider response loss remains `AMBIGUOUS`, is not blindly resent, and is deliberately ineligible for generic duplicate-risk Manual Retry. Already accepted upload work may finish its deterministic Telegram notification even after the invite becomes exhausted, without reopening the capability for new files.

## D-034 — Use one Telegram Bot identity for Admin private chat and Support forum Topics

**Decision:** The primary Telegram webhook uses one Bot identity. After webhook path, secret-token and update validation, authorized private chats route to the existing Admin control plane; only the exact configured forum supergroup routes to Support Topic processing. Unauthorized private users and every other Telegram chat context produce no Support Queue side effect. Admin authorization remains the bootstrap-only `ADMIN_TELEGRAM_USER_IDS` allowlist.

**Identity rule:** Unified Admin UI must not rotate its own Bot token/webhook identity. It may still migrate `BOT_GROUP_ID`, manage notification/runtime settings and refresh the existing webhook. Bot identity changes are deployment/bootstrap operations. The legacy `/webhooks/admin-telegram/...` route and legacy Admin bootstrap secrets may remain temporarily for migration/rollback compatibility.

**Reliability compatibility:** `TELEGRAM_SUPPORT_PROFILE` and `supportProfileVersion` are retained without a new migration. The profile version continues to scope Support Queue identity, operator ordering, target evidence and stale-generation fencing. `admin_update_receipts` remains a separate Admin dedupe domain; sharing Telegram's global `update_id` sequence does not combine Admin and Support receipt identity. Retirement of legacy profile/version state is explicitly future work.

## D-035 — Permit guarded one-click online rotation while preserving a single active Telegram Bot

**Decision:** D-035 supersedes only the identity-rotation restriction in D-034; D-034's one-Bot routing, authorization and reliability model remains in force. Online Bot rotation is an approved Admin capability. The candidate must be distinct from the current effective Unified/Support Bot, the Bot handling the Admin interaction and the legacy `ADMIN_TELEGRAM_BOT_TOKEN`. Before confirmation, the control plane validates candidate identity, the configured forum supergroup and topic-management permission. Confirmation creates a fresh webhook path/secret, calls `setWebhook` with `message`, `edited_message` and `callback_query` plus `drop_pending_updates=true`, then advances `TELEGRAM_SUPPORT_PROFILE` through the existing CAS/version semantics. The new generation immediately serves both authorized private Admin and Support forum traffic.

**Single-active-Bot rule:** After the new profile becomes authoritative, the previously active Support webhook and the legacy Admin webhook are automatically disabled with `deleteWebhook(drop_pending_updates=false)`. Retirement targets are deduplicated and the new Bot is never a retirement target. A successful rotation therefore leaves exactly one active CZ2128 Telegram Bot. Old BotFather identities and bootstrap credentials may remain as offline emergency-recovery material, but their provider webhooks are inactive. No manual webhook refresh and no separate legacy-Admin retirement action is required after success.

**Failure boundary:** Candidate validation or candidate `setWebhook` failure changes neither D1 profile state nor old provider webhooks. If candidate `setWebhook` succeeds but profile activation/CAS fails, the candidate webhook is best-effort deleted and the old profile remains authoritative. Once profile activation succeeds, the new Bot remains authoritative even if an old webhook cannot be retired; that state is explicitly reported as incomplete rather than success. Authenticated Admin session state records only the prior profile version and remaining retirement categories, never raw tokens, and can retry only those remaining `deleteWebhook` operations without repeating profile rotation or asking for the new token again. Ordinary webhook refresh remains `drop_pending_updates=false`, because refresh is not a generation boundary. No D1 migration is required.

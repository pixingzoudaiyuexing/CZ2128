...
**Decision:** Phase 4B-4B supports explicit recovery only for `internal / ai_trigger`. It is not generic replay. The original `source_event_ref` is reused exactly, and the event payload contains only the validated canonical conversation ID and durable Chatwoot message reference. Eligibility requires an OPEN `cz2128-dlq` receipt, matching reclaimable `event_receipts`, the newest durable customer text, an existing matching `ai_runs` row, enabled AI, equal handoff epochs and no active generation owner. External messages, lifecycle events, `attachment_transfer` and quarantine evidence remain non-redrivable.

**AI and handoff rule:** Only due `FAILED_RETRYABLE` runs below the existing three-attempt limit and `SUCCESS` runs with durable response text are eligible. Attempts and retry history are never reset. Retry reclaim requires the durable run epoch to equal the current conversation epoch inside the claim CAS. A newer handoff epoch permanently fences old work even after AI is re-enabled. Old retryable work is cancelled through the existing handoff semantic; historical SUCCESS remains truthful and is not regenerated.

**Outbound rule:** SUCCESS recovery checks deterministic Chatwoot and Telegram AI operation identities against an explicit whitelist. No operation, `PENDING`, due in-budget `FAILED_RETRYABLE` and `SENT` are safe candidates. `SENDING`, `AMBIGUOUS`, `FAILED_FINAL`, malformed evidence, identity conflicts, exhausted attempts and future retry deadlines fail closed. AMBIGUOUS recovery remains exclusively in reconciliation/Mark Delivered/Cancel/Manual Retry workflows.

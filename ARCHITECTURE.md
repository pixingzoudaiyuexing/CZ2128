...
The Admin allowlist remains bootstrap-only. Unified Admin Telegram settings may migrate `BOT_GROUP_ID`, manage notification/runtime settings and safely refresh the active webhook, but cannot create or confirm `TELEGRAM_SUPPORT_PROFILE` Bot rotation. Bot identity is a deployment/bootstrap concern in unified mode so a failed self-rotation cannot simultaneously remove Admin and Support access.

For compatibility, `/webhooks/admin-telegram/...` and its legacy bootstrap settings may remain during migration/rollback. `TELEGRAM_SUPPORT_PROFILE` and `supportProfileVersion` also remain intact: their version still scopes Support event identity (`tg:<supportProfileVersion>:<updateId>`), operator ordering and stale-generation fencing. Admin receipts and Support events use separate persistence domains, and the shared Telegram Bot's globally unique update sequence does not merge those dedupe stores. Removing the legacy profile/version fields is future work, not part of this compatibility change.

...
The Support Telegram profile is one encrypted atomic value containing token, webhook secret and webhook path. Its monotonically increasing runtime version is also the Support Bot generation: Telegram Queue/event/message/outbound identity and conversation operator ordering are scoped by `(support_profile_version, update_id)`. Queue events from an older generation are dropped before side effects and future-generation events retry. One-click rotation validates a genuinely new Bot and the current forum group/topic permissions, creates a fresh webhook path/secret, sets the candidate webhook with `drop_pending_updates=true`, and atomically activates it through the existing versioned D1 CAS. Only after activation does the control plane retire the previous Support webhook and the legacy Admin webhook with `deleteWebhook(drop_pending_updates=false)`, deduplicating identical old identities and never deleting the new Bot. If candidate `setWebhook` fails, old state remains untouched; if D1 activation fails after candidate webhook creation, the candidate webhook is best-effort removed. If old-webhook retirement is incomplete after activation, the new profile remains authoritative and an authenticated Admin session retries only the remaining retirement targets without re-running rotation. Ordinary webhook refresh remains `drop_pending_updates=false`. Support-group migration validates the active bot against a forum supergroup and atomically changes `BOT_GROUP_ID` while clearing existing Telegram topic mappings. Neither side-effectful key is exposed through generic rollback.

### 18A. Single Telegram Bot Routing

The current Telegram topology uses one Bot identity and one primary `/webhooks/telegram/<path>` webhook. Webhook path verification, `X-Telegram-Bot-Api-Secret-Token` verification and `update_id` validation happen before chat-context routing. Authorized private chats are handled synchronously by the existing Admin control plane and deduplicated by `admin_update_receipts`; they never become Support Queue events. Only `chat.type=supergroup` with exact `chat.id=BOT_GROUP_ID` enters the existing Support Topic path. Unauthorized private users, ordinary groups, channels, missing-chat updates and unrelated supergroups are accepted/ignored without Support Queue side effects.

The Admin allowlist remains bootstrap-only. Unified Admin Telegram settings expose guarded `轮换客服 Bot`, group migration, notification/runtime settings and safe webhook refresh. Candidate identity checks cover the current effective Unified/Support token, the current Admin interaction Bot and the legacy Admin bootstrap token. The confirmation message states that the new Bot will immediately take over both private Admin and the Support forum while the old Support and legacy Admin webhooks are disabled. A successful confirmation requires all retirement work to finish; otherwise Admin receives an explicit partial state and a retry path rather than a false success.

For compatibility, `/webhooks/admin-telegram/...` code and its legacy bootstrap credentials may remain for break-glass recovery, but a successful online rotation disables that legacy Bot's provider webhook; credentials existing offline is not treated as an active second Bot. `TELEGRAM_SUPPORT_PROFILE` and `supportProfileVersion` remain intact: their version still scopes Support event identity (`tg:<supportProfileVersion>:<updateId>`), operator ordering and stale-generation fencing. Admin receipts and Support events use separate persistence domains, and the shared Telegram Bot's globally unique update sequence does not merge those dedupe stores. Removing the legacy profile/version fields is future work, not part of this compatibility change.

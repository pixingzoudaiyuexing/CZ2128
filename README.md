...
- A newer handoff epoch blocks recovery at Admin validation and inside retry reclaim CAS. AI OFF then ON cannot revive old historical work.
- SUCCESS recovery permits only no operation, `PENDING`, due in-budget `FAILED_RETRYABLE` or `SENT`; `SENDING`, `AMBIGUOUS`, `FAILED_FINAL` and inconsistent evidence fail closed. AMBIGUOUS stays in reconciliation/Manual Retry.
